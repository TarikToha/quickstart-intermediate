...
class ConsoleProgressBar
{
     ...
     * @return  void
     */
    protected function generateProgress($done, $total, $size = 30, $vol)
    {
        ...
        $eta = round($rate * $left);
        if ($rate > 0) {
            $rate = round($vol / ($rate * 100000), 1);
        }

        $statusBar .= str_repeat("&emsp;", 5) . "remaining: " . $this->convertSecondsToHMS($eta) .
            str_repeat("&emsp;", 5) . "elapsed: " . $this->convertSecondsToHMS($elapsed) .
            str_repeat("&emsp;", 5) . "speed: " . $rate . " kBps";
     ...
     * @return  void
     */
    public function showProgress($limit, $total, $size = 30, $vol)
    {
        $this->currentCount += $limit;
        return $this->generateProgress($this->currentCount, $total, $size, $vol);
    }
}